...
// CHANGE CHART BACKGROUND COLOR & 0 SCALE
chartBackground.addEventListener("input", function () {
  const selectedValue = this.value;
  graph.style = `background-color: ${selectedValue}`;
});

// CHANGE SQUARE COLORS SCALE 1 - 4
...
function getColorValues() {
  let scale2Value = window
    .getComputedStyle(scale2, null)
    .getPropertyValue("background-color");
  let scale3Value = window
    .getComputedStyle(scale3, null)
    .getPropertyValue("background-color");
  let baseValue = window
    .getComputedStyle(scale4, null)
    .getPropertyValue("background-color");
  let scale1Value = window
    .getComputedStyle(scale1, null)
    .getPropertyValue("background-color");
  let scale0Value = window
    .getComputedStyle(scale0, null)
    .getPropertyValue("background-color");
